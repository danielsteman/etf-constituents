from main import IsharesFundsListScraper, IsharesFundScraper


class TestIsharesFundsListScraper:
    def test_get_fund_list(self):
        fund_list_scraper = IsharesFundsListScraper(
            "https://www.ishares.com/nl/professionele-belegger/nl/producten/etf-investme\
                nts#/?productView=all&dataView=keyFactspageNumber=1"
        )
        fund_list = fund_list_scraper.get_funds_list()
        assert len(fund_list) > 0


class TestIsharesFundScraper:
